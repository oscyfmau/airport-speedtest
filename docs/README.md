...
版本：v4.43.0 ｜ 仓库：[github.com/oscyfmau/airport-speedtest](https://github.com/oscyfmau/airport-speedtest) ｜ [更新记录](CHANGELOG.md)
...
1. **下载**：打开 [Releases 页面](https://github.com/oscyfmau/airport-speedtest/releases)，下载最新版的 `airport-speedtest-v4.42.0.zip`（精简包，含运行所需的全部文件）并解压，解压后先看里面的 `使用教程.txt`（会用 git 也可以 `git clone`）
...
- 不用 git：到 [Releases 页面](https://github.com/oscyfmau/airport-speedtest/releases) 下载最新版 `airport-speedtest-v4.42.0.zip`（精简包，仅含运行核心文件）并解压；需要自行改代码时再下载 `Source code (zip)`
- 用 git：
...
### 配色含义（v4.43.0 MiaoKo 蓝绿视觉）

| 列 | 色块 |
|---|---|
| 延迟RTT / HTTP延迟 / 网页均耗 | 整格色块：快=亮绿 `#4DD06F` → 中=黄绿 `#8BC34A` → 慢=深橙 `#FF7000`（帧间线性插值）；`超时`/`--`/`UDP`/`代理可达` 为浅灰块 `#E9E9E9` |
| 平均/最高速度 | 整格色块（MiaoKo 蓝绿冷色系）：慢=浅绿 `#B9E67D` → 中=蓝 → 快=深蓝 `#0D9AF2`；按节点本身速度 log2 评分（参考上限 25MB/s，超过即最蓝；高速区平缓、整体色差小）；无速度时显示失败原因（白底黑字） |
| 流媒体 | 解锁/可用=浅绿、待解锁/仅自制剧=柔和黄、失败/封锁/连接失败=柔粉、N/A/查询失败=中灰、未知=灰、跳过(节点不可达)=深灰、未测=无底色 |
| IP类型 | 家宽/移动=浅绿、商宽/机房=柔和黄、代理/VPN/Tor=柔粉 |
| IP风险 | 低(<20)=浅绿、中(20-59)=柔和黄、高(60+)=柔粉 |
| 复用 | 完全复用=深红、中转复用=深黄、落地复用=深青 |
| 每秒速度柱 | 约 10 根紧密排列（任意长度采样重采样为 10 点）；柱高=该秒绝对速度/全表最大归一化（底部对齐，反映真实快慢）、柱色=绝对速度（与速度格同蓝绿色板、跨行可比）；无每秒数组节点显示 10 根等高矮柱 |
...
- 报告版式：标题栏/页脚浅灰 `#EBEBEB`，表头与数据区纯白；列间淡竖线 `#E6E6E6`、浅灰细外框 `#C8C8C8`（无深黑粗框）
- 页眉第 1 行：`speed_test.py vX.Y.Z | 模式名`（居中加粗）；第 2 行：`订阅: N 节点 | 测试耗时: Xs`（左）、`排序: 排序名`（右）
- 页脚第 1 行：绿勾「已核实TLS证书」+ 延迟术语说明（quick 模式为 `快速模式（并行近似测速）`）
...
- 订阅分组条（v4.33.0）：选中 ≥2 份订阅时，报告按订阅分组绘制——每组上方一条 `订阅 N（M 节点）` 通栏浅灰横条（#EBEBEB），组间以横条分隔；组序=订阅顺序，组内按所选排序；序号跨组连续
