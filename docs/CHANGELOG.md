...
报告渲染改为独立模块（MiaoKo 风格渲染器）；修复代码审查确认的 5 项 P1、25 项 P2 与若干 P3 级缺陷。

### 新增

- `core/miaoko_report.py`（新模块）：MiaoKo 风格报告渲染器——下载速度表（红粉）+ 流媒体检测表（蓝绿 + 状态色）。导出 `render_miaoko_report` / `draw_download_table` / `draw_streaming_table` / `build_download_rows` / `build_streaming_rows` / `build_streaming_columns` / `load_font` / `parse_speed` / `parse_ms` / `draw_sparkline` 及 `_resample_k` / `_ping_text` / `_ip_type_color` / `_ip_risk_color` / `_reach_stats` / `_with_group_markers` / `_DL_NAME_WIDTH`
- `report.generate_report_image` 新增 `truncated` 参数（页脚"仅显示前 N/M 节点"），并按 mode 委托渲染：speed/basic/quick → 下载速度表；normal/full/streaming/streaming_ai/streaming_all → 流媒体检测表
- `report` 增加 `from __future__ import annotations`（允许 PEP 604 注解写法，同时保持 Python 3.9 可导入）
- `profiles`：`_is_unlocked` / `_countable_streaming`（解锁率统一判定）
- `parser`：vless/vmess 输出 `h2-opts` / `http-opts`；wireguard 支持从 query 读 `publicKey` / `public-key`
- `cli`：`_read_text_auto`（utf-8-sig → gbk 自动探测）、`_merge_urls`（订阅 URL 追加去重）、`_safe_mtime`
- `engine`：`_mask_log_text`（子进程 stderr 文本遮蔽）
- `tester`：测速窗口看门狗（首字节到达后启动，睡满窗口即关窗）

### 修改

- 排序：`sort_results` 改由 `report.generate_report_image` 调用（v4.33.0 语义不变：none/default = 订阅顺序）；修复委托渲染后"选了排序方式但表格仍按订阅顺序"
- PNG 行数上限恢复为 300 行，超出时页脚追加"仅显示前 N/M 节点"（v4.40.0 语义）
- 页脚第二行恢复可达统计：`节点: 可达数/总数 | 平均延迟: Nms [| UDP节点: N 个(经HTTP实测)] | 排序=…`，取代写死的 `概要=n/n`
- quick 模式页脚第一行为"快速模式（并行近似测速）"（不再声称已核实 TLS 证书）
- 多订阅（≥2 个 sub_index）时插入分组横条"订阅 N（M 节点）"；序号列只对节点行计数
- 每秒速度柱：归一化基准由"全表平均速度最大值"改为"全表最大瞬时速度"；柱数重采样为 10 根；每秒数据为空时不再生成示意柱（`generate_speeds` 保留导出，真实路径不再调用）
- 下载表节点名改按像素截断（`_trunc_to_width`，列宽常量 `_DL_NAME_WIDTH = 290`）
- 流媒体表：IP 类型 / IP 风险列按 `IP_TYPE_COLORS` 与 `STREAMING_STATUS_COLORS`（ok/pending/fail）上色；ASN 与状态文本按列宽像素截断；未收录的服务 id 回退 `config.FULL_STREAMING_SERVICES` 的 `name`
- 解锁状态取色：仅完全等于"未知"才判未知（`解锁(未知)` 走解锁色）；裸状态码 `(NNN)` 归"未知"色（`miaoko_report.streaming_color` 与 `report._stream_block_color` 同口径）
- IP 类型文本：风控字段（机房/代理/移动）全为 None 时返回 "-"（不再输出"家宽IP"）
- 无速度数据的行，"每秒速度"格子与相邻速度列同为灰底 (200,200,200)
- `parser`：vmess/ssr 解析前剥离 `#fragment`；SSR 参数键名 `obfsparam`→`obfs-param`、`protoparam`→`protocol-param`，且 protoparam 走 base64 解码（失败回退明文）；SSR 节点名优先级 remarks > group > server；`ss://` 的 SIP002 插件拆为 `plugin` + `plugin-opts`（`obfs`→`mode`、`obfs-host`→`host`）；`https://` 补 `tls: true`（hostname 为域名时补 `sni`）；端口统一校验 1-65535；`ss://` 明文 userinfo 按最后一个 `@` 切分；trojan/hysteria2/anytls 密码含 `:` 时拼回完整 userinfo；wireguard 缺省端口 443 → 51820
- `cli`：`-i` 文件与位置 URL 合并（去重保序），解析不到 URL 时明确提示、不再静默进菜单；代理.txt 读写按 utf-8-sig → gbk 自动探测并保持原编码（BOM/GBK 均可）；结果对比/分组对比编号加 1..N 校验（0/越界不再命中负索引）；内核更新临时目录移入 `finally` 清理；报告列表取 mtime 统一走 `_safe_mtime`；删除报告失败时打印失败清单（不再谎报已删除）；`_MODE_NAMES` 补 `"quick": "快速检测"`
- `settings`：保存改 tmp + `os.replace` 原子写（tmp 名带 pid + 毫秒）；文件损坏回退默认时写一条 warning
- `profiles`：解锁率统一判定（`失败/错误/跳过/未知/封锁/不可用` 不计入解锁，分母剔除 `跳过` 与 `错误`）；证据折叠新增 `speed_n` / `reach_total` 计数，`_weighted_stats` 分母改用"带该维度的条数"（旧档案缺字段时回退旧口径）；`append_run` 与 `rebuild_profiles` 增加本轮已归档去重（同一 run 同一档案只写一条 appear/evidence）
- `streaming`：`check_tiktok` / `check_spotify` / `check_steam` / `check_primevideo` / `check_max` 的 403 归为"封锁"（与 check_disney / check_chatgpt / check_claude / check_perplexity / check_gemini 一致）
- `ip_quality`：ip-api.com 返回 `status: "fail"` 视为失败并继续回退下一源（此前被当成功、不再回退）；令牌桶改为每次真实请求取令牌（含重试），实发速率不再达配置值 2 倍
- `engine`：mihomo stderr 每行过 `_mask_log_text` 后落盘；换端口重试前清理上一份临时配置（含凭据的 yaml）；`_find_free_port` 端口段耗尽返回 0，`start()` 明确报"无可用端口"
- `tester`：`test_node_speed` 增加窗口看门狗——首字节到达后睡满窗口即 `finished.set()`，链路黑洞（无 RST、无 chunk）不再白等满 `HTTP_DOWNLOAD_TIMEOUT`；此类节点（窗口内下载量 < `MIN_SPEED_BYTES`）归因为"速度过低"

### 修复

- `core/engine.py`：内核下载 SHA-256 校验分支永远不可达——`_requests.get(zip_path + ".sha256")` 传的是本地落盘路径而非下载 URL（抛 `InvalidSchema` 被 except 吞成"校验不可用"），v4.35.0 声明的"存在同名 .sha256 即核验、不符即删除中止"实际从未生效；改为取远端 `url + ".sha256"`，并区分"取校验文件失败"与"官方未提供"
- `core/report.py`：Python 3.9 导入即 `TypeError`——`_get_speed_color(s: float | None)` 使用 PEP 604 注解且文件无 `from __future__ import annotations`（与 README / run.bat 声明的 Python 3.9+ 冲突）
- 下载速度表节点名溢出到相邻列：按字符数截断 26 字符（26 个全角字符约 494px）超出 290px 的列宽
- 流媒体列标题出现裸小写服务 id（13 个未收录服务，如 crunchyroll / soundcloud）
- `parser`：`vmess://<b64>#香港01` 等带非 ASCII fragment 的链接整条节点被静默丢弃；SSR 的 `obfsparam` / `protoparam` 键名不被 mihomo 识别且 protoparam 未解码；`ss://` 明文密码含 `@` 被错切
- `cli`：`-i` 与位置订阅 URL 相互静默覆盖；代理.txt 为 GBK（Windows 记事本 ANSI）时添加/删除订阅报错且提示与真实原因不符
- `settings`：非原子写（先截断再写）+ 损坏静默回退默认，用户设置可能无声丢失
...
- 报告列宽估算纳入表头标题宽（`generate_report_image` 列宽循环补 `font.getlength(title)`）——流媒体列标题（YouTube 69px / B站港澳台 74px 等）不再被默认列宽截断；UDP类型 62→67px 等窄列相应微宽
- 节点数超 300 上限时输出 WARNING："PNG 仅显示前 300 个（完整数据见同名 JSON）"
- `_flag_to_text` docstring 修正：v4.8.1 起 PNG 报告节点名同样经过本函数（仅文件日志/JSON 保留原名）
- `_get_speed_color`/`_bar_color` 补函数级弃用注释（v4.32.0 起报告改走 `_speed_color`，保留为兼容接口）

### 修复

- 流媒体列表头（如 "YouTube"、"B站港澳台"）宽于默认列宽时表头文字被截断
- 节点数超 300 时无任何提示（用户不知 PNG 被截断、完整数据在 JSON）
...
- R16：IP/网页列空数据不渲染（`has_ip`/`has_web` 门控在 v4.32.0 布局已生效）
- R18：空结果路径不会调用报告生成（runner 早退护栏 + 告警已有）
...
- **产物累积清理**（`profiles.cleanup_outputs`）：output/ 保留最近 keep_reports 份 PNG+JSON 配对（10/30/100 默认 30）、log/ 保留最近 keep_logs_days 天（7-365 默认 30）；护栏（只匹配 测速结果_*/测速日志_* 前缀、成对删、profiles.json 永不参与、单文件异常跳过）；自动清理在 run_test 与_finish_partial 收尾静默执行（事件 `profiles_cleanup`）；维护页 3 手动清理（先 dry_run 统计占用与将删数再确认执行）
