...
版本：v4.44.0 ｜ 仓库：[github.com/oscyfmau/airport-speedtest](https://github.com/oscyfmau/airport-speedtest) ｜ [更新记录](CHANGELOG.md)
...
1. **下载**：打开 [Releases 页面](https://github.com/oscyfmau/airport-speedtest/releases)，下载最新版的 `airport-speedtest-v4.44.0.zip`（精简包，含运行所需的全部文件）并解压，解压后先看里面的 `使用教程.txt`（会用 git 也可以 `git clone`）
...
- 菜单管理：节点稳定性视图（近 N 次出现率/可达率/波动）、节点筛选测速（按关键字/前 N 个）、历史结果管理（打开/删除）、订阅管理（遮蔽显示/添加/删除）、设置页（窗口秒数/并行数/自动开报告/稳定性窗口/大流量确认/保留报告份数/日志保留天数，跨会话保存）、环境信息页
...
- 不用 git：到 [Releases 页面](https://github.com/oscyfmau/airport-speedtest/releases) 下载最新版 `airport-speedtest-v4.44.0.zip`（精简包，仅含运行核心文件）并解压；需要自行改代码时再下载 `Source code (zip)`
- 用 git：
...
| `python core/speed_test.py <URL> --quick` | 快速检测（并行近似测速 + 4 个核心流媒体，单节点约 10MB 流量） |
...
报告配色遵循直觉色（绿/蓝=快/好、橙/粉=慢/差），所有文字纯黑直绘、无描边，报告内不印图例；整格底色色块表达快慢好坏，数字只做精确读数。

### 配色含义（v4.44.0 MiaoKo 渲染器：下载速度表红粉 + 流媒体检测表蓝绿）

| 列 | 色块 |
|---|---|
| HTTPS延迟 | 整格色块（两表共用）：快=亮绿 `(70,200,80)` → 黄绿/黄 → 橙 → 慢=红 `(220,45,15)`（按 200/400/650/900/1200/1600ms 分档）；`-`（无数据）为灰块 `(200,200,200)` |
| TLS RTT（流媒体表） | 整格色块：快=亮绿 `(60,230,60)` → 黄 → 橙 → 慢=红 `(220,40,15)`（按 80/120/180/250/350/500ms 分档）；`UDP`/`代理可达`/`超时` 无 ms 数值 → 不取色 |
| 平均/最高速度（下载表） | 整格色块（红粉表）：浅青 `(187,216,217)` → 浅蓝/蓝灰/淡紫 → 紫 → 粉红 → 品红 `(245,35,118)`；分档 1/5/10/15/22/32/45/65MB/s（≤10MB/s 偏冷色、>22MB/s 转粉红）；无速度时显示失败原因（白底黑字） |
| 平均/最高速度（流媒体表） | 整格色块（蓝绿表）：浅青 `(200,235,250)` → 亮蓝 → 深蓝 `(30,140,230)`；分档 1/5/10/20/40/70MB/s |
| 流媒体 | 解锁/可用=浅绿、待解锁/仅自制剧=柔和黄、失败/封锁/连接失败=柔粉、N/A/查询失败=中灰、未知=灰、跳过(节点不可达)=深灰、未测=无底色；裸状态码 `(NNN)` 归“未知”色 |
| IP类型 | 家宽/移动=浅绿、商宽/机房=柔和黄、代理/VPN/Tor=柔粉；风控字段全为空（数据源无风控数据）时不填色 |
| IP风险 | 低(<20)=浅绿、中(20-59)=柔和黄、高(60+)=柔粉 |
| 复用 | 完全复用=深红、中转复用=深黄、落地复用=深青（有复用数据时才建列） |
| 每秒速度柱 | 10 根（任意长度重采样到 10 点）；柱高=该秒速度/全表最大瞬时速度（底部对齐，反映真实快慢）；柱色随绝对速度（下载表粉红渐变 / 流媒体表蓝色渐变、跨行可比）；无每秒数据的行不画柱，该格与相邻速度列同为灰底 |
...
| `HTTP延迟 超时` | 延迟探测请求（gstatic）未得到 204 响应，或该节点无此数据（v4.39.0 起文档与报告一致显示「超时」） |
...
| 每秒速度柱状图 | 约 10 根柱（任意长度采样重采样为 10 点）；柱高=该秒绝对速度/全表最大归一化（底部对齐，反映真实快慢）、柱色=绝对速度（慢=浅绿→快=深蓝，与速度格同蓝绿色板、跨行可比）、柱间 1px 白缝；无每秒数组节点显示 10 根等高矮柱；quick 模式同样绘制（v4.33.0 起） |
...
OpenAI/Claude/Perplexity 判别（v4.33.0）：三者的网页端点对数据中心 IP + 非浏览器请求有 Cloudflare 机器人风控（支持区域的节点也会被 403 误报"封锁"），改用 API 端点判定——`api.openai.com`、`api.anthropic.com`、`api.perplexity.ai` 上只要请求到达 API（401/400/405/429/5xx 等业务错误码）即为区域放行，403 为区域封锁；放行时经 `cdn-cgi/trace` 标注出口地区。v4.34.0 起三检测器语义统一（此前 chatgpt 对 429/5xx 会落回网页探测链，与 claude/perplexity 同节点三列自相矛盾），且 API 不可达时回退网页探测但不再产出"封锁"（网页 403 无法区分区域封锁与机器人风控，一律显示 `未知`）。v4.43.0 起 Gemini 同用 API 端点判别——`generativelanguage.googleapis.com/v1beta/models` 带无效 key，403=封锁、其余已到达（401/400/429/5xx）=可用；网页 gemini.google.com 兜底判"封锁"降"未知"。
...
- 报告版式（MiaoKo 风格）：标题栏 `#EBEBEB` 浅灰、表头 `#E8E8E8`/`#E4E4E4`、数据区 `#FCFCFC` 与 `#F8F8F8` 交替、网格线 `#C8C8C8`/`#E6E6E6`、页脚 `#E8E8E8`
- 标题行：`MiaoKo 风格 | 模式名 | speed - 下载速度`（非下载表模式为 `test - 自定义测试`）
- 页脚第 1 行：绿圆勾 + `已核实 TLS 证书。TLS RTT 为单次数据交换延迟，HTTPS Ping 为单次请求体感延迟。`（quick 模式为 `快速模式（并行近似测速）`，不再声称已核实证书）
- 页脚第 2 行：`节点: 26/33 可达 | 平均延迟: 234ms`（有 UDP 节点时追加 `| UDP节点: 4 个(经HTTP实测)`，末尾 `| 排序=排序名`）；`可达` = 直连成功 + 隧道探测成功之和 / 节点总数；`平均延迟` = 直连 TCP 成功节点的平均延迟
- 页脚第 3 行：`测试时间: YYYY-MM-DD HH:MM:SS (时区), 本次实测下载 X | 引擎=speed_test.py vX.Y.Z, 本测试为试验性结果，仅供参考。`；节点数超过 300 时中间追加 `| 仅显示前 300/N 节点`（v4.40.0 语义、v4.44.0 恢复）
- 订阅分组条（v4.33.0 起，v4.44.0 恢复）：选中 ≥2 份订阅时，报告按订阅分组绘制——每组上方一条 `订阅 N（M 节点）` 通栏浅灰横条，组间以横条分隔；组序=订阅顺序，组内按所选排序；序号列只对节点行计数

## 排序方式

直接回车 = 订阅顺序（v4.33.0 起为默认；选中多份订阅时报告按订阅分组绘制，组内按所选排序）。可选：最大/平均速度升序或降序、节点名 A→Z / Z→A。
...
- 报告最多显示前 300 个节点
- 油管下载源默认隐藏（`core/config.py` 里 `YOUTUBE_SOURCE_ENABLED = False`）；如需启用第 4 个测速源（googlevideo 直链），改为 `True` 并安装 yt-dlp
- 测速会消耗节点流量（每节点约 10-30MB），"勿跑大流量"节点请谨慎全量测试
- shadowtls/naive/juicity 类型节点仅被解析、mihomo 不支持会被过滤（防单条不支持类型导致整份配置加载失败），不进测速流程（v4.39.0 起文档注明）
- 网页模拟测速会在标准/完整测试中为每个节点额外增加约 3-8 秒（4 站点并发、8 秒超时）；`--fast` 模式跳过
- `-h`/运行均需先安装依赖（未装依赖时入口 import 失败；run.bat 会先自动安装依赖再运行；手动 `pip install -r core/requirements.txt` 亦可）
- TCP 丢包率为 3 次握手的失败计数，对瞬时抖动敏感，仅作参考
- 平台支持（v4.31.0）：Linux / macOS 未经实测——mihomo 内核自动下载已修复（Windows `.zip` / Linux/macOS `.gz` 格式，x86_64/arm64 架构，下载与解压路径已实测验证）；macOS 手动下载 mihomo 放入 `bin/` 会被 Gatekeeper 拦截（"无法验证开发者"），请用首次运行自动下载或菜单 `6 更多 → 9 维护 → 1 更新 mihomo 内核`；Linux 无图形界面时报告不会自动打开（`xdg-open` 不存在，不影响测试与手动查看 `output/`），无中文字体时 PNG 报告中文显示为方框（可安装 Noto Sans CJK）
